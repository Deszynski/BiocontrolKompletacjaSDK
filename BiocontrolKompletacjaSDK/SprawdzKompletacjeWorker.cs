                ...
                using (var workbook = new XLWorkbook())
                {
                    ...
                    using (Session session = Context.Login.CreateSession(false, false))
                    {
                        ...
                        foreach (PozycjaDokHandlowego poz in dokument.Pozycje)
                        {
                            ...
                            j = towaryModule.Jednostki.WgKodu[symbol];
                            ...
                            switch (precyzjaPoprawna)
                            {
                                ...
                                case false:
                                    ...
                                    worksheet.Cell(currentRow, 8).Style.Fill.BackgroundColor = XLColor.Crimson;
                                    break;
                            }

                            // sprawdzenie stanow magazynowych
                            /*StanMagazynuWorker worker = new StanMagazynuWorker
                            {
                                Towar = towar,
                                Data = dokument.Data
                            };

                            double stan = worker.StanRazem.Value;*/

                            zasoby = towar.Zasoby.CreateView();
                            zasoby.Condition &= new FieldCondition.LessEqual("Partia.Data", dokument.Data);
                        
                            sumaZasobow = 0;
                            foreach (Zasob z in zasoby)
                            {
                                if (!(z.Partia.Dokument.Numer.ToString().Contains("ZO") || z.Partia.Dokument.Numer.ToString().Contains("ZD")))
                                    sumaZasobow += z.IlośćZasobu.Value;
                            }

                            worksheet.Cell(currentRow, 9).Value = sumaZasobow;

                            stanPoprawny = sumaZasobow - ilosc >= 0;
                            ...
                            switch (stanPoprawny)
                            {
                                ...
                                case false:
                                    ...
                                    worksheet.Cell(currentRow, 10).Style.Fill.BackgroundColor = XLColor.Crimson;
                                    break;
                            }
                            ...
                            foreach (ElementKompletu element in towar.ElementyKompletu)
                            {
                                if (element.Towar.Nazwa.ToString() != towar.Nazwa.ToString())
                                {
                                    ...
                                    // sprawdzenie stanow magazynowych
                                    /*StanMagazynuWorker workerElem = new StanMagazynuWorker
                                    {
                                        Towar = element.Towar,
                                        Data = dokument.Data
                                    };
                                    ...
                                    double stanElem = workerElem.StanRazem.Value;*/

                                    zasoby = element.Towar.Zasoby.CreateView();
                                    zasoby.Condition &= new FieldCondition.LessEqual("Partia.Data", dokument.Data);
                                    ...
                                    stanPoprawny = sumaZasobow - ilosc * poz.Ilosc.Value >= 0;
                    ...
                    worksheet.Cell(1, 9).Value = "Zasób na dzień\n " + dokument.Data.ToString();
                    worksheet.Cell(1, 10).Value = "Zasób pozwala na operację";
